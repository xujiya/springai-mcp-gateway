# Security Audit Report: HAProxy:8080 Public Endpoint

**Audit Date:** 2026-08-10 (封板 v0.14.0)  
**Target:** http://localhost:8080 (HAProxy)  
...
The HAProxy:8080 endpoint serves as the sole public entry point, proxying to api-gateway:8081 and mcp-gateway:8082. 
...
- HAProxy del-header Server, root / → default_backend api_gw
...
**Description:**  
Internal services bound to host-accessible ports, bypassing HAProxy and gateway security:
...
| M3 | nginx version leaked | del-header Server (HAProxy) | v0.7.0 |
...
- [ ] **TLS**: HTTPS on HAProxy with valid certificate
